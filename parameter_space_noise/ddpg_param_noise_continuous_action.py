...
from parameter_noise import AdaptiveParamNoiseSpec, ddpg_distance, perturb_model
...
"""
    ...
    perturbed_actor = Actor(envs).to(device)
    perturbed_actor.load_state_dict(actor.state_dict())
    adaptation_actor = Actor(envs).to(device)
    ...
    perturbed_actor.load_state_dict(actor.state_dict())
    perturb_model(perturbed_actor, param_noise)
    ...
    for global_step in range(args.total_timesteps):
        ...
        if global_step < args.learning_starts:
            ...
            with torch.no_grad():
                actions = (
                    perturbed_actor(torch.Tensor(obs).to(device), train=False)
                    .cpu()
                    .numpy()
                )
        ...
        if global_step > args.learning_starts:
            ...
            with torch.no_grad():
                next_state_actions = target_actor(data.next_observations, train=False)
                qf1_next_target = qf1_target(
                    data.next_observations, next_state_actions, train=False
                )
                next_q_value = data.rewards.flatten() + (
                    1 - data.dones.flatten()
                ) * args.gamma * (qf1_next_target).view(-1)

            qf1_a_values = qf1(data.observations, data.actions, train=True).view(-1)
            ...
            if global_step % args.policy_frequency == 0:
                actor_loss = -qf1(
                    data.observations, actor(data.observations, train=True), train=False
                ).mean()
            ...
            if global_step % args.adaptation_frequency == 0:
                ...
                with torch.no_grad():
                    unperturbed_actions = actor(data.observations, train=False)
                    adaptation_actions = adaptation_actor(
                        data.observations, train=False
                    )
                distance = ddpg_distance(
